...
//Цикл while
//Цикл while работает немного иначе от for, он проверяет условие на истинность виразу, що задана в тілі умови. А фор
//просто перевіряє умову в частю В і тоді виходить із циклу. 
	$i = 5;
	while ($i <= 10) {
		echo ++$i."<br>";
	}
...
//Форми
	...
	//ACTION - вказується куди відправляти дані(шлях до файлу), якщо цей атрибут буде пустий - 
	//це буде означати, що дані відсилатимуться самому собі, це те саме, 
	//якби в ньому написати ім'я файлу в якому це атрибут і прописується.
	//METHOD - Get, Post. Якщо параметр не вказаний, то по замовчуванню задається параметр Get.
	//INPUT - призначений для створення текстових полів, різних кнопок, перемикачів і прапорців.

//$_GET/$_POST
	//Це суперглобальний масив, в який попадають дані, що проходять в ПХП через адресну строку,
	//з використанням методу GET/POST. Який буде name переданої строки, такий ж буде ключ масиву,
	//в який буде прийматись значення цього елементу.

	echo '<p>Моє імя: '.$_GET['name'];
	echo '<p>Моє імя: '.$_POST['name'];

//Використання методів GET, POST
...
//Оголошення глобальних змінних: global $a
...
//Метод HEAD
    //Це метод - той самий, що і GET, однак передаються лише заголовки файлу, що запрошуються, без тіла.
    //Тобто, якщо нам наприклад потрібно дізнатись розмір файлу, тоді нам буде достатньо цього методу.
?>
